...
############################################
# Ultra-Advanced NMT Trainer (Fixed Version)
############################################

class LocalNMTTrainer:
    """
    Fixed NMT Trainer with stable BLEU optimization.

    Improvements:
    - Correct padding pipeline
    - Stable validation decoding
    ...
    # --------------------------------------------------
    # Dataset Loader (Quality Filtered)
    # --------------------------------------------------
    ...
    def load_dataset(self):
        ...
        train_matches = sorted(
            DATA_DIR.glob(f"wmt*_{src_lang}_{tgt_lang}.train.jsonl"),
            reverse=True
        )

        test_matches = sorted(
            DATA_DIR.glob(f"wmt*_{src_lang}_{tgt_lang}.test.jsonl"),
            reverse=True
        )

        train_path = train_matches[0] if train_matches else None
        test_path = test_matches[0] if test_matches else None

        def load_pairs(path, max_n=50000):
        ...
        train_pairs = load_pairs(train_path, 50000)
        test_pairs = load_pairs(test_path, 3000)

        # Synthetic fallback
        if not train_pairs:

            logger.info(f"Using synthetic dataset for {self.lang_pair}")
            ...
            train_pairs = [random.choice(templates)
                           for _ in range(10000)]

            test_pairs = random.sample(templates, 4) * 750
    ...
    # --------------------------------------------------
    # Beam Search Decoder (BLEU Boost)
    # --------------------------------------------------

    def beam_search_decode(self, model, src_tensor,
                           bos_id, eos_id,
                           beam_size=4,
                           max_len=50,
                           temperature=0.8):
        ...
        beams = [
            [(torch.full((1,), bos_id, device=device), 0.0)]
        ...
        with torch.no_grad():
            ...
            for _ in range(max_len):
                ...
                for b_idx in range(batch_size):
                    ...
                    for seq, score in beams[b_idx]:

                        probs = log_probs[b_idx]

                        topk = torch.topk(probs, beam_size)

                        for token, logp in zip(topk.indices, topk.values):

                            new_seq = torch.cat(
                                [seq, token.unsqueeze(0)]
                            )

                            candidates.append(
                                (new_seq, score + logp.item())
                            )
                ...
                if all(
                    any(seq[-1].item() == eos_id for seq, _ in beam)
                    for beam in beams
                ):
                    break
        ...
        for beam in beams:
            ...
            final_outputs.append(best_seq.tolist())
            ...
            for i in range(0, len(train_pairs), batch_size):
                ...
                for src, tgt in batch:
                    ...
                    try:
                        ...
                        if len(src_id) < 3 or len(tgt_id) < 3:
                            continue
